...
public class CustomSmileRating extends BaseRating {
    ...
    private String[] mNames = getResources().getStringArray(com.hsalf.smilerating.R.array.names);
...
//    private Path mEyePathRight = new Path();
    ...
    private void parseAttrs(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, com.hsalf.smilerating.R.styleable.SmileRating);
            mAngryColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_angryColor, mAngryColor);
            mNormalColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_normalColor, mNormalColor);
            mDrawingColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_drawingColor, mDrawingColor);
            mPlaceHolderSmileColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_placeHolderSmileColor,
                    mPlaceHolderSmileColor);
            mPlaceholderBackgroundColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_placeHolderBackgroundColor,
                    mPlaceholderBackgroundColor);
            mTextSelectedColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_textSelectionColor,
                    mTextSelectedColor);
            mTextNonSelectedColor = a.getColor(com.hsalf.smilerating.R.styleable.SmileRating_textNonSelectionColor,
                    mTextNonSelectedColor);
            mShowLine = a.getBoolean(com.hsalf.smilerating.R.styleable.SmileRating_showLine, true);
            mIndicator = a.getBoolean(com.hsalf.smilerating.R.styleable.SmileRating_isIndicator, false);
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int a = getMaxNameLineNumber();

        setMeasuredDimension(Math.round(mWidth), (int) Math.round(mHeight + (mHeight * 0.48 * getMaxNameLineNumber())));
        createTouchPoints();
    ...
    private int getMaxNameLineNumber() {
        Integer[] names = {getLines(getSmileName(GREAT)),
                getLines(getSmileName(GOOD)),
                getLines(getSmileName(OKAY)),
                getLines(getSmileName(BAD)),
                getLines(getSmileName(TERRIBLE))};

        List<Integer> b = Arrays.asList(names);
    ...
    private void drawTextCentered(String text, float x, float y, Paint paint, Canvas canvas) {
        for (String line: text.split("\n")) {
            float xPos = x - (paint.measureText(line) / 2);
            float yPos = (y - ((paint.descent() + paint.ascent()) / 2));
            canvas.drawText(line, xPos, yPos, mTextPaint);
